...
## Statut actuel (2026-08-05 02:45) — server 3.237.0 / client 3.575.0 / desktop 3.21.0 (release publiée)

**Cycle 42 (`users.rs::get_user`, fuite `birthday`/`totp_enabled`) vérifié en conditions réelles au cycle 44** : `UserPublic` filtrait déjà `activity_*` selon `activity_visibility` de la cible, mais renvoyait `birthday` (date de naissance exacte) et `totp_enabled` sans filtrage à N'IMPORTE QUEL viewer, y compris un étranger complet sans amitié. Repro live confirmé avant fix (`200`, date exacte renvoyée). Fix : mêmes deux champs nullifiés dans le bloc `if user_id != claims.sub` déjà existant. Reconfirmé après déploiement : `birthday: null` pour un spectateur non lié.
...
**Cycle 33 (`friends.rs::open_dm`, réglage `dm_from_all` placebo)** : 4e instance du pattern "réglage de confidentialité jamais appliqué côté serveur" trouvé cette session (après channel_permissions, verification_enabled, et maintenant celui-ci). `open_dm` ne vérifiait ni les blocages ni `dm_from_all` de la cible -- n'importe qui pouvait ouvrir un salon DM avec n'importe qui, même après désactivation explicite de "Autoriser les DMs de tout le monde". Repro live confirmé (`200` malgré `dm_from_all=false` et aucune amitié). Fix : même pattern que `send_friend_request` (blocks + check ami si `dm_from_all=false`), salons déjà existants non affectés (seule la création est gatée). **Déploiement confirmé au cycle 34** : `verify_dm_privacy.mjs` relancé après rebuild → `403` (rejeté correctement) au lieu du `200` d'avant.
...
## ⚠️ Finding NON corrigé — réglage "Afficher mon statut en ligne" (`show_online`) jamais appliqué (2026-08-05, cycle 34)

Même famille que les findings placebo ci-dessous, mais **cas particulier** : contrairement à `channel_permissions`/`verification_enabled`/`dm_from_all` (corrigé ce cycle), il existe déjà un mécanisme PLEINEMENT FONCTIONNEL pour le même objectif -- le sélecteur de statut manuel (`UserPanel.tsx`, options En ligne/Absent/Ne pas déranger/**Invisible**) est correctement câblé de bout en bout (`websocket.rs::handle_socket` lit `users.status='invisible'` à la connexion, `broadcast_presence`/`PRESENCE_INIT` respectent ce statut, filtré aux amis + membres de serveurs communs). Le toggle `show_online` de `PrivacySection.tsx` ("Afficher mon statut en ligne" / "Les autres peuvent voir si vous êtes connecté") promet exactement le même résultat mais n'est **jamais lu nulle part côté serveur** (grep exhaustif `show_online` sur `server/src` : seulement les 3 sites CRUD de `user_settings.rs`, zéro consultation dans `websocket.rs` ou ailleurs) -- désactiver ce toggle n'a strictement aucun effet, l'utilisateur reste visible.

**Pourquoi pas corrigé en autonome, contrairement aux 3 findings similaires déjà réglés ce cycle** : (1) une vraie correction changerait un comportement de visibilité réel pour tout utilisateur ayant DÉJÀ désactivé ce toggle en pensant qu'il fonctionnait -- même risque que `verification_enabled`, pas auto-fixable sans décision produit ; (2) **question de design non triviale, pas juste un oubli d'implémentation** : contrairement aux autres findings, une solution alternative complète existe déjà (statut "Invisible") -- faut-il (a) faire de `show_online=false` un simple alias serveur de `status='invisible'`, (b) le laisser distinct pour permettre "en ligne mais visible seulement des amis" (une sémantique différente de invisible-pour-tous), ou (c) retirer purement et simplement ce toggle redondant de `PrivacySection.tsx` puisque "Invisible" couvre déjà le besoin ? Les 3 options ont des implications UX différentes, à trancher par Momo plutôt que par un correctif autonome minimal.
...
## Finding NON corrigé — Kanban (`KanbanBoard.tsx`) : le tableau est TOUJOURS vide dans les 4 colonnes (2026-08-05, cycle 46)

**Sévérité fonctionnelle la plus haute trouvée cette session** (pas un enjeu de sécurité, mais une fonctionnalité 100% cassée en usage normal, pas juste un edge case). Le client (`KanbanBoard.tsx`) modélise les tâches avec un champ `status: 'todo' | 'in_progress' | 'review' | 'done'` et filtre chaque colonne par `tasks.filter(t => t.status === col.id)`. **Mais le serveur (`moderation.rs`, table `channel_tasks`) n'a JAMAIS eu de colonne `status`** -- seulement un `completed BOOLEAN` (+ `priority`). `GET /channels/:id/tasks` ne renvoie donc jamais de champ `status` du tout. Conséquence directe, déterministe (pas besoin de repro complexe, confirmé quand même en direct) : `t.status` vaut `undefined` pour CHAQUE tâche récupérée, `undefined === 'todo'`/`'in_progress'`/`'review'`/`'done'` est toujours faux -- **aucune tâche n'apparaît JAMAIS dans AUCUNE des 4 colonnes**, même quand des tâches existent réellement en base (confirmées créées avec succès côté API, juste invisibles dans l'UI Kanban). Repro live : `GET /channels/:id/tasks` après création d'une tâche → `200`, tâche bien présente dans la réponse JSON, mais avec les clés `id/channel_id/title/description/completed/assignee_id/due_date/priority/creator_id/created_at/updated_at` -- **`status` absent, `completed` présent**, confirmant la lecture de code.
  **2e bug indépendant trouvé dans le même composant, corrigé ce cycle (celui-là oui, sûr et minimal)** : le sélecteur de priorité client propose `'low'|'medium'|'high'|'urgent'` (défaut `'medium'`), mais le serveur valide `["low","normal","high","urgent"]` (`CHECK` SQL en plus de la validation applicative) -- **`'medium'` n'a jamais été une valeur acceptée**, donc créer une tâche SANS changer la priorité par défaut échouait TOUJOURS avec `400 "priority invalide"`. Repro live confirmé (`400` avant fix). Fix appliqué : renommé `'medium'` → `'normal'` dans le type/état/liste d'options du client (libellé affiché "Medium" conservé, seule la valeur sous-jacente envoyée à l'API change) -- alignement pur avec un contrat serveur déjà existant, aucun changement de schéma, aucune décision de produit nécessaire. C'est un fix isolé et sûr, contrairement au problème `status`/`completed` ci-dessus.
  **Pourquoi le bug `status`/`completed` n'est PAS corrigé en autonome** : aucune solution minimale et sûre n'existe. Deux options réelles, toutes deux hors du périmètre d'un correctif autonome : (a) ajouter une vraie colonne `status` côté serveur (migration DB, **explicitement interdit sans l'accord de Momo**) pour donner enfin un support réel aux 4 colonnes annoncées par l'UI ; (b) réduire le client à un modèle à 2 états (Todo/Terminé) calé sur le `completed: bool` réel -- ce qui reste une vraie décision de produit/UX (supprimer 2 des 4 colonnes visibles), pas un simple patch, et un correctif "à moitié" (ex. faire cohabiter les 3 colonnes non-"done" sur le même `completed=false` sans vraie distinction) réintroduirait une UX confuse (une tâche glissée vers "En cours" reviendrait visuellement à "Todo" au prochain refetch, puisque le serveur ne mémorise rien de plus qu'un booléen) plutôt que de réparer proprement. À trancher avec Momo : implémenter le vrai `status` (migration) ou simplifier l'UI à 2 colonnes.

## Finding NON corrigé — `channel_permissions` (overrides par rôle/membre) est une fonctionnalité MORTE, jamais appliquée (2026-08-14, cycle 50)

**Sévérité potentiellement élevée, découverte en cherchant un bug plus étroit** (fuite d'activité `pin_rows` dans `get_activity_feed`, `users.rs`, qui ne filtre pas par canal visible -- symptôme initial). En creusant la cause, la table `channel_permissions` (overrides `allow`/`deny` par rôle ou par membre sur un canal précis, gérée via `GET/PUT/DELETE /channels/:id/permissions` dans `channels.rs`, UI probable = un onglet "Permissions" dans les réglages de canal) **n'est lue NULLE PART ailleurs que dans son propre handler `get_channel_permissions`** (qui sert juste à l'afficher dans l'UI de configuration). Grep exhaustif confirmé : `channel_permissions` n'apparaît dans AUCUNE des 55 occurrences de `require_member_and_channel`/`require_permission_and_channel`/`require_channel_in_server` (12 fichiers : channels, messages, threads, forum, polls, feeds, saved, scheduled, uploads, webhooks, websocket, servers) ni dans `get_channels` (liste des canaux d'un serveur, `channels.rs::get_channels`, filtre uniquement par `server_id` + membership serveur). Un admin peut donc configurer "refuser Voir le canal au rôle X sur #canal-prive" dans l'UI, la sauvegarde réussit (200), et **l'override n'a strictement aucun effet** : le rôle X voit toujours le canal dans la sidebar, peut toujours lire/poster/réagir dedans, comme n'importe quel autre canal du serveur. Seule la permission de RÔLE globale (`VIEW_CHANNEL` sur le rôle lui-même, appliquée via `require_permission`) compte réellement -- il n'existe aujourd'hui aucun moyen de rendre un canal privé à un sous-ensemble de membres d'un serveur.

**Pourquoi pas corrigé en autonome** : même catégorie de risque que `verification_enabled` (cycle 11) déjà classé "NON corrigé, décision Momo requise" -- **activer rétroactivement l'enforcement d'un système de permission jusqu'ici totalement inerte peut couper l'accès à des canaux pour des membres qui y écrivent activement aujourd'hui**, sur un nombre inconnu de serveurs en prod ayant peut-être déjà configuré des overrides `deny` en pensant (à tort) qu'ils étaient actifs. Contrairement à `quiet_hours` (fixé sans hésiter, cycle 42 -- purement additif, désactivé par défaut, ne retire jamais un accès existant), ceci retirerait potentiellement un accès à des utilisateurs actifs sans avertissement -- exactement le type de changement que `require_permission_and_channel`/`require_member_and_channel` centralisent déjà (bon point : un vrai fix serait localisé à ces 2-3 fonctions partagées dans `servers.rs` + un filtre dans `get_channels`, pas 12 fichiers à toucher un par un -- mais l'ampleur du changement de comportement observable reste hors du périmètre "correctif minimal et sûr" sans validation explicite). À trancher avec Momo : (a) implémenter l'enforcement réel (le fix technique est en fait assez contenu grâce à la centralisation existante), ou (b) si la fonctionnalité n'est pas utilisée en pratique, retirer l'UI de configuration des channel_permissions pour ne pas laisser un faux sentiment de contrôle d'accès.
**Sous-symptôme mineur associé, pas corrigé séparément** (même cause racine, un patch isolé serait trompeur) : `get_activity_feed` (`users.rs`, `pin_rows`) affiche les épinglages de TOUS les canaux du serveur à TOUS les membres du serveur sans vérifier l'accès au canal précis -- cohérent avec le fait qu'aucun canal n'est jamais réellement restreint aujourd'hui, redeviendra pertinent le jour où (a) sera implémenté.

## Finding NON corrigé — révoquer une session efface aussi son "historique de connexion" (2026-08-14, cycle 54)

**Réel, sécurité/audit, nécessite une migration DB** (colonne manquante) donc hors périmètre autonome. `list_sessions` (`auth.rs`, alimente `SessionsSection.tsx` -- "Sessions actives") et `get_login_history` (`users.rs`, alimente `LoginHistorySection.tsx` -- explicitement labellé côté UI "20 dernières connexions à votre compte") sont **exactement la même requête sur la même table** `user_sessions`, sans aucune distinction. `revoke_session` (déjà corrigé une fois, cf. commentaire en place sur le vrai bug refresh_token) fait un `DELETE FROM user_sessions WHERE id=$1` -- ce qui retire la ligne des DEUX vues à la fois. Conséquence concrète : un utilisateur qui repère une session suspecte (IP/appareil inconnu) dans "Sessions" et clique "Révoquer" (réaction naturelle et recommandée en cas de compromission) **efface du même geste la trace de cette connexion dans "Historique de connexion"** -- au moment précis où cette trace serait utile (identifier quand/d'où l'intrusion a eu lieu). Un log d'audit qui disparaît quand on répond à l'alerte qu'il vient de donner est un défaut de conception, pas un edge case.
**Pourquoi pas corrigé en autonome** : la seule vraie réparation est de séparer les deux usages (état vivant vs. journal permanent) via un champ genre `revoked_at TIMESTAMPTZ NULL` sur `user_sessions` -- `list_sessions`/`SessionsSection` filtrerait `revoked_at IS NULL`, `get_login_history` verrait tout sans filtre, `revoke_session` ferait un `UPDATE ... SET revoked_at=NOW()` au lieu d'un `DELETE`. Migration de schéma, explicitement hors périmètre "correctif minimal et sûr" sans accord de Momo. Un correctif partiel (ex. ne plus DELETE la ligne `user_sessions` dans `revoke_session`) ne ferait que déplacer le bug : "Sessions actives" afficherait alors des sessions mortes comme si elles étaient encore vivantes -- pas une vraie réparation, juste un déplacement du même défaut de conflation vers l'autre écran.
**Observation secondaire liée, même cause racine** (pas un bug à part, pas de section dédiée) : `SessionsSection.tsx` détermine la "session actuelle" en prenant `sessions[0]` (tri `ORDER BY last_seen DESC`) -- une heuristique positionnelle, pas une identification garantie. `last_seen` n'est mis à jour qu'au refresh du token (pas à chaque requête), et les `Claims` JWT (`middleware/auth.rs`) ne portent aucun `session_id` permettant de savoir avec certitude QUELLE ligne `user_sessions` correspond à la requête en cours. Dans le cas rare où un autre appareil a rafraîchi son token plus récemment que l'appareil réellement utilisé pour consulter les Paramètres, le mauvais appareil serait marqué "Session actuelle" (badge vert) -- risque limité (pas de fuite de données, au pire l'utilisateur révoque la mauvaise session et se retrouve déconnecté, se reconnecte). Fix propre = ajouter un `session_id` aux Claims JWT ou lire le cookie refresh_token -- touche le format du token ou le scope des cookies, catégorie "changement d'architecture auth", hors périmètre autonome au même titre que le point principal ci-dessus.
