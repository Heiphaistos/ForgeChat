...
**Vérification cycle 26 en attente** : fix `scheduled.rs` poussé (commit 0655603) mais `cargo build --release` CI toujours en cours au moment de la rédaction -- pas encore de test fonctionnel réel en prod. À faire au cycle suivant avant de considérer ce fix confirmé (voir procédure dans la section cache Docker ci-dessous).
