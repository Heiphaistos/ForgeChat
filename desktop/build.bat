...
title ForgeChat Desktop — Build v3.12.0

echo.
echo ============================================
echo   ForgeChat Desktop Builder v3.12.0
echo ============================================
echo.
...
for /r "%BUNDLE%\nsis" %%f in (*.exe) do (
    if !FOUND_NSIS!==0 (
        copy "%%f" "%OUT%\ForgeChat-Setup-v3.12.0.exe" >nul
        echo [OK] Installeur : dist-desktop\ForgeChat-Setup-v3.12.0.exe
        set FOUND_NSIS=1
    )
)
if %FOUND_NSIS%==0 (
...
if exist "%SCRIPT_DIR%src-tauri\target\release\forgechat-desktop.exe" (
    copy "%SCRIPT_DIR%src-tauri\target\release\forgechat-desktop.exe" "%OUT%\ForgeChat-Portable-v3.12.0.exe" >nul
    echo [OK] Portable  : dist-desktop\ForgeChat-Portable-v3.12.0.exe
) else (
